...
- Markdown 文章列表、创建、编辑、删除，以及当前区块编辑、其余区块实时渲染的一体化编辑器。
...
- 自适应桌面侧栏和移动端底部导航。
- 首次启动欢迎向导，可逐项检测 Git、Cloudflare R2 与 Starry Blog 状态，也可跳过。
- Markdown 编辑工具栏支持标题、引用、表格、代码、强调、列表与链接，并保留选中文本。
- 支持将文章设为私密并配置访问密码；密码仅写入 Markdown 源文件，不进入文章索引。
- 可自定义应用主色，支持跟随系统、手动夜间模式与纯黑模式。
- WordPress WXR 导入：预检文章、页面、私密内容、评论与媒体；HTML 转 Markdown，按日期和标题拼音生成 slug，顺序迁移引用媒体到 R2，并以单次 Git 提交写入博客。

## WordPress 导入

在侧栏打开“WP 导入”，选择 WordPress 导出的 WXR/XML 文件并先执行预检。导入器只处理文章、页面、被引用媒体及已批准评论；菜单、主题、站点配置和未知自定义内容类型会跳过。

- 媒体默认从导出文件中的原主机下载。测试环境可指定实际连接 IP；跳过 SSL 仅作用于本次导入和指定源主机。
- 下载与 R2 上传严格顺序执行。对象以内容 SHA-256 命名，映射保存在仓库的 `.starrynote/wordpress-media.json`，中断后可以继续并避免重复上传。
- 可选择在源媒体返回 404 时保留原 URL 并继续导入；缺失数量会显示在结果中，其他网络错误仍会中止。
- WordPress 私密内容需要在导入前设置统一访问密码。Starry Blog 的私密文章属于静态站轻量保护，不能替代服务端鉴权。
- 评论回复会扁平化为 Markdown 引用；重复执行时会跳过已存在的文章、媒体和相同评论。
- 导入前要求 Git 工作区干净。成功后只创建一个本地提交，仍需手动 Push。
